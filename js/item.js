//maxX prend pour valeur la longueur du terrain
function Items (id, maxX, y, field) {
    this.id     = id;
    this.x      = Math.floor(Math.random()*maxX);
    this.y      = y;
    this.field  = field;

    var items = document.createElement("div");
    items.setAttribute("class", "items");
    items.setAttribute("id", this.id);
    document.body.appendChild(items);
}

//affiche l'item
items.prototype.display = function () {
    var HTMLitems = document.getElementById(this.id);
    HTMLitems.style.top = (this.y*50) + "px"; //devra être adapté automatiquement à la hauteur du sol
    HTMLitems.style.left = (this.x*70) + "px";
}

//lui fait trouver le sol
items.prototype.findGround = function () {
    var nextY = this.y + 1;
    //on situe x par rapport au tableau
    var actualX = this.x-1;

    if(this.field.checkBloc(nextY, actualX) == 0) {
        this.y += 1;
        nextY += 1;
        return true;
    } else {
        return false;
    }
}